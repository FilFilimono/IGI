...
class AdvancedAnalyzer(TextProcessor):
    """
    Advanced text analyzer using RegEx.
    Demonstrates: Inheritance, RegEx, string manipulation.
    """
    def __init__(self, content):
    ...
    def get_average_lengths(self):
        ...
        # Sentences for avg length calculation
        ...
        avg_sent_len = 0
        if total_sent > 0:
            # Count only word characters in sentences
    ...
    def count_smileys(self):
        """Counts specific smiley pattern."""
        # Pattern: [;:] + [-]* + same bracket (one or more)
        pattern = r'[;:][-]*(\(|\)|\[|\])\1*'
        matches = re.finditer(pattern, self.content)
        # We need to filter to ensure it's ONLY the same bracket
        count = 0
    ...
    def individual_task_processing(self):
        """Solves specific RegEx tasks from the assignment."""
        # 1. Capital English letters
        cap_letters = "".join(re.findall(r'[A-Z]', self.content))
        
        # 2. Replace p...pb...bc...c with ddd
        # p+ (at least one), b{2,} (more than one), c+ (at least one)
...
def run_task_2():
    """Main function for Task 2 execution."""
    print("\n--- Task 2: Text Analysis ---")
    
    # Mocking file read for demo - in real scenario use open()
